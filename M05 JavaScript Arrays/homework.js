...
function arrayContiene(array, elemento) {
   ...
   // Tu código:
   var estaElemento = array.includes(elemento);
   return estaElemento;
}
...
function numeroMasGrande(arrayOfNums) {
   ...
   for (let i = 0; i < arrayOfNums.length; i++) {
      if (numMayor < arrayOfNums[i]) numMayor = arrayOfNums[i];
   }
   return numMayor;
}
...
function empiezaConNueve(num) {
   ...
   // Tu código:
   var numstr = num + 'cadena';
   var arrayNum = numstr.split('')
   if (arrayNum[0] == 9) {
      return true;
   } else return false;
}
...
function mesesDelAño(array) {
   ...
   if (!incluyeEnero || !incluyeMarzo || !incluyeNoviembre) {
...
function tablaDelSeis() {
   ...
   var resTablaDelSeis = [];
   for (let i = 0; i <= 10; i++) {
      resTablaDelSeis.push(6*i);
   }
   return resTablaDelSeis;
}
